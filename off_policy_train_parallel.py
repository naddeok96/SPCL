#==========================================
# File: off_policy_train_parallel.py
#==========================================
...
def _unwrap_module(module: nn.Module) -> nn.Module:
    """Return the underlying module when wrapped by DataParallel/DDP."""
...
def _state_dict(module: nn.Module):
    """Get a plain state dict regardless of parallel wrapping."""
    return _unwrap_module(module).state_dict()
...
def _parse_gpu_ids(config: dict, override: str | None) -> List[int]:
    """Resolve GPU ids from CLI override or config file."""
    if override:
        ids = [int(x) for x in override.split(",") if x.strip()]
    else:
        mg = config.get("multi_gpu", {})
        # allow keys `gpu_ids` or `gpus`
        ids = mg.get("gpu_ids") or mg.get("gpus") or []
...
def _configure_primary_device(config: dict, gpu_ids: Sequence[int]) -> torch.device:
    """Update config with the primary device and return it."""
    if gpu_ids and torch.cuda.is_available():
        available = torch.cuda.device_count()
        invalid = [g for g in gpu_ids if g < 0 or g >= available]
        if invalid:
            raise ValueError(
                f"Requested GPU ids {invalid} but only {available} visible CUDA devices."
            )
        primary = int(gpu_ids[0])
        device_str = f"cuda:{primary}"
...
def _maybe_wrap_multi_gpu(agent: DDPGAgent, gpu_ids: Sequence[int]) -> bool:
    """Wrap agent networks in DataParallel if multiple GPUs requested."""
    use_multi = (
    ...
    device_ids = [int(g) for g in gpu_ids]
    output_device = device_ids[0]

    agent.actor = nn.DataParallel(agent.actor, device_ids=device_ids, output_device=output_device)
    agent.critic1 = nn.DataParallel(agent.critic1, device_ids=device_ids, output_device=output_device)
    agent.critic2 = nn.DataParallel(agent.critic2, device_ids=device_ids, output_device=output_device)

    # OUNoise operates on primary device; no change required.
    print(f"Enabled DataParallel on GPUs: {device_ids}")
    return True
...
def main():
    ...
    parser.add_argument("--config", "-c", default="config_off_policy_parallel.yaml",
                        help="path to your config YAML")
    parser.add_argument("--gpus", type=str, default=None,
                        help="Comma-separated GPU ids to use (e.g. 0,3,5)")
    args = parser.parse_args()
    config = load_config(args.config)

    gpu_ids = _parse_gpu_ids(config, args.gpus)
    primary_device = _configure_primary_device(config, gpu_ids)
    ...
    multi_gpu_enabled = _maybe_wrap_multi_gpu(agent, gpu_ids)
    if not multi_gpu_enabled and len(gpu_ids) > 1:
        print("Multiple GPUs requested but unavailable; proceeding on primary device.")
    ...
    for update in trange(num_updates, desc="Off-policy Training"):
        ...
        if time.time() - last_hour_save >= 3600:
            torch.save(_state_dict(agent.actor), hourly_actor)
            torch.save(_state_dict(agent.critic1), hourly_critic1)
            torch.save(_state_dict(agent.critic2), hourly_critic2)
        ...
        if update % evaluation_interval == 0:
            ...
            if mean_reward > best_mean_reward:
                ...
                torch.save(_state_dict(agent.actor),  os.path.join(results_dir, "best_actor.pth"))
                torch.save(_state_dict(agent.critic1), os.path.join(results_dir, "best_critic1.pth"))
                torch.save(_state_dict(agent.critic2), os.path.join(results_dir, "best_critic2.pth"))
                no_improve = 0
            else:
                no_improve += 1
            ...
            if update % checkpoint_interval == 0:
                torch.save(_state_dict(agent.actor),  os.path.join(results_dir, f"off_policy_actor_{update}.pth"))
                torch.save(_state_dict(agent.critic1), os.path.join(results_dir, f"off_policy_critic1_{update}.pth"))
                torch.save(_state_dict(agent.critic2), os.path.join(results_dir, f"off_policy_critic2_{update}.pth"))
    ...
    torch.save(_state_dict(agent.actor), final_actor)
    torch.save(_state_dict(agent.critic1), final_critic1)
    torch.save(_state_dict(agent.critic2), final_critic2)
